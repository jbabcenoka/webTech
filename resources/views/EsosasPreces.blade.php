...
@section('content')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="second" id="tmp">
                            ...
                            <input type="search" id="search" placeholder= '{{ __('messages.find_zieds') }}' oninput="showproducts()" style="width:75%" >
                                    ...
                                    <div class='second' >
                                        <h2 id="{{$i}}">
                                            @switch($flower->ZiedaPuskaVeids)
                                                ...
                                                @case('Peonijas'){{ __('messages.Peonijas') }} @break
                                                @case('Puskis Flora'){{ __('messages.Puskis_Flora') }} @break
                                                @case('Puskis Luiza') {{ __('messages.Puskis_Luiza') }}  @break
                                                @case('Puskis Maja'){{ __('messages.Puskis_Maja') }}  @break
                                                @case('Puskis Milestiba') {{ __('messages.Puskis_Milestiba') }}  @break
                                                @case('Puskis Pavasara'){{ __('messages.Puskis_Pavasara') }} @break
                                                @case('Puskis Vasara'){{ __('messages.Puskis_Vasara') }} @break
    ...
    <script>
        ...
        function showproducts(){
            ...
            let u=0;
            for(let j=0; j<20; j++){
                q = document.getElementById(j);
                var parentDiv = q.parentNode;
                parentDiv = parentDiv.parentNode;
                var id = parentDiv.getAttribute("id");
                aa[u]=id; u++;
            }
            var name = document.getElementById("search").value; ///Atrast ievadita lauka vardu
            for(let i=0; i<20; i++){ ///Meklet starp visam esosam precem
                var container = document.getElementById(aa[i]); ////parbaudit katru preci
                var k = document.getElementById(i).innerHTML;
                if(container.hidden==false && name.length>=1){ ///Ja ievadits kaut viens burts
                    if(k.toLowerCase().includes(name)) { container.hidden=false;} ///Satur search mekleto vardu
                    else container.hidden=true; ///Ja nav, tad paslept
                }
                if(name.length<1){ ///Atgriezt visu atpakal
                    for(let i=0; i<20; i++){
                        var k = document.getElementById(i).innerHTML;
                        var container = document.getElementById(aa[i]);
